...
def _load_cfg():
    with open("configs/splitter.yaml", "r", encoding="utf-8") as f:
        # return a raw dict so split_into_subclaims can normalize nested dicts
...
def _boundary_pr(gold: list[int], pred: list[int]) -> tuple[float, float, float]:
    # SPECIAL CASE: if both sides are empty, count as perfect for this item.
...
def _prt_pr(gold_spans: list[list[int]], pred_spans: list[list[int]]) -> tuple[float, float, float]:
    # exact-span matching; your splitter’s PRT is already strict
...
def main() -> None:
    ...
    with open(args.dev, "r", encoding="utf-8") as fh:
        for i, line in enumerate(fh):
            ...
            out = split_into_subclaims(q, getattr(cfg, "splitter", cfg))
